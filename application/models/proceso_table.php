...
class ProcesoTable extends Doctrine_Table {
    ...
    public function findProcesosDisponiblesParaIniciar($usuario_id, $cuenta = 'localhost', $orderby = 'id', $direction = 'desc') {
        ...
        $cuenta = Doctrine::getTable('Cuenta')->find(1);//UsuarioBackendSesion::usuario()->cuenta_id);
        $estados = "'public'";
    ...
    public function findProcesosExpuestos($cuenta_id) {
        if (strlen($cuenta_id)>0) {
            ...
            $sql = "select p.id, p.nombre, p.version, p.estado, t.nombre as tarea, t.id as id_tarea, t.exponer_tramite, t.previsualizacion,c.id as id_cuenta, c.nombre_largo as nombre_cuenta from proceso p, tarea t, cuenta c 
                    where p.id = t.proceso_id and p.cuenta_id=c.id and t.exponer_tramite=1 and p.activo=1 and p.estado!='arch' and p.cuenta_id=".$cuenta_id.";";
        }else{
            log_message('info','No tiene id');
            $sql = "select p.id, p.nombre, p.version, p.estado, t.nombre as tarea, t.id as id_tarea, t.exponer_tramite, t.previsualizacion,c.id as id_cuenta, c.nombre_largo as nombre_cuenta from proceso p, tarea t, cuenta c 
                    where p.id = t.proceso_id and p.cuenta_id=c.id and t.exponer_tramite=1 and p.activo=1 and p.estado!='arch';";
        }
    ...
    public function findVariblesFormularios($proceso_id, $tarea_id){
        $sql="select f.nombre as nombre_formulario, c.id as variable_id, c.nombre as nom_variables "
                . " from proceso p,tarea t, paso pa, formulario f, campo c "
                ...
                . " GROUP by f.nombre, c.id, c.nombre;";
        $stmn = Doctrine_Manager::getInstance()->connection();
        $result = $stmn->execute($sql)
        ->fetchAll();
        return $result;
    ...
    public function findVariblesProcesos($proceso_id){
        $sql = "select a.id as variable_id, a.nombre as nombre_variable, a.extra, p.nombre as nombre_proceso from accion a, proceso p, tarea t where a.proceso_id=p.id and a.tipo='variable' and p.activo=1 and a.proceso_id=".$proceso_id." and p.id=t.proceso_id group by a.id, a.nombre, a.extra, p.nombre;";
        $stmn = Doctrine_Manager::getInstance()->connection();
